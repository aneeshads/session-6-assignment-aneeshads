...
def test_session_6_check_docstring_less_than_50_characters():
    ...
    assert session6.check_docstring()(test_func_docstring_less_than_50_characters) == False, "The function is not working for docstring with less than fifty characters"
...
def test_session_6_check_docstring_greater_than_50_characters():
    ...
    assert session6.check_docstring()(test_func_docstring_greater_than_50_characters) == True, "The function is not working for docstring with more than fifty characters"


####################### Validation for get_fibonacci_number #######################################

def test_session_6_get_fibonacci_number_type_check():
    '''Test to check if the input number is greater than zero; the test also checks the number is an integer; the test also checks the number is a Fibonacci one.'''
...
def test_session_6_track_arth_count_function_execution_test():
    ...
    for k in range(8):
        div_fn(10, 8)
    assert (session6.dictionary_of_functions['add'] == 3) and (session6.dictionary_of_functions['mul'] == 5) and (session6.dictionary_of_functions['div'] == 8), "Function not working correctly"
...
def test_session_6_new_input_dictionary_inner_no_args():
    '''Test to check if the mandatory positional arguments have been passed in the inner function of new_input_dictionary'''
    with pytest.raises(TypeError, match=r".*required positional arguments*"):
        session6.new_input_dictionary()()
